...
import numpy as np
import torch
DETECTION_COLOR_MAP = {'Car':(0,255,200), 'Pedestrian':(0,200,35), 'Cyclist':(116,100,200)}
...
class Draw3DBox:
    def __init__(self, box3d_pub, marker_frame_id = 'map', rate=10, clock=None):
    ...
    def publish_3dbox(self, dt_box_lidar, track_ids, types=None, publish_id=True, publish_type=True, ):
        ...
        for i, corners_3d_velo in enumerate(corners_3d_velos):
            ...
            if publish_id and track_ids != -1:
                ...
                text_marker.header.stamp = self.clock.now().to_msg()

                text_marker.id = track_id + 1000
                text_marker.action = Marker.ADD
                text_marker.lifetime = Duration(sec=int(LIFETIME), nanosec=int((LIFETIME % 1) * 1e9))
