...
class PcdetNode(Node):
    def __init__(self):
            ...
            self.frame = 0
            self.cam_pub = self.create_publisher(Image, '/camera_image', 10)
            self.kitti_pub = self.create_publisher(Image, '/kitti_image', 10)
            self.kitti_depth = self.create_publisher(Image, '/kitti_depth', 10)
            self.detect_3dbox = self.create_publisher(MarkerArray, '/detect_3d_box', 10)
            self.PCL = self.create_publisher(PointCloud2, '/velodyne_points', 10)
            self.cam_sub = self.create_subscription(Image, '/camera_image', self.cam_subscribe_callback,  10)
            self.lidar_sub = self.create_subscription(PointCloud2, '/velodyne_points', self.lidar_callback, 10)
            self.proc_1 = Processor_ROS(cfg_root, model_path, self.get_clock())
            self.proc_1.initialize()
            self.proc_1.set_pub_rviz(self.detect_3dbox, marker_frame_id = FRAME_ID)
            self.timer = self.create_timer(0.1, self.publish_data)  # 每 0.1 秒呼叫一次
    
    def publish_data(self):
            image_publish(self.frame, self.kitti_pub)
            publish_pcl(self.frame, self.PCL, self.get_clock())
            # cam_publish(self.frame, self.cam_pub)
            self.frame += 1
    ...
    def cam_subscribe_callback(self, data):
        ...
        # Display image
        self.get_logger().info(f'Camera frame {self.frame} received.')
        cv.imshow("camera", current_frame)
        cv.waitKey(1)

    def lidar_callback(self, msg: PointCloud2):
        select_boxs, select_types = [],[]
        if self.proc_1.no_frame_id:
            self.proc_1.set_viz_frame_id(msg.header.frame_id)
            print(f"{bc.OKGREEN} setting marker frame id to lidar: {msg.header.frame_id} {bc.ENDC}")
            self.proc_1.no_frame_id = False

        frame = msg.header.stamp.sec # frame id -> not timestamp
        msg_cloud = ros2_numpy.point_cloud2.pointcloud2_to_array(msg)
        np_p = get_xyz_points(msg_cloud, True)
        scores, dt_box_lidar, types, pred_dict = self.proc_1.run(np_p, frame)
        for i, score in enumerate(scores):
            if score>threshold:
                select_boxs.append(dt_box_lidar[i])
                select_types.append(pred_dict['name'][i])
        if(len(select_boxs)>0):
            self.proc_1.pub_rviz.publish_3dbox(np.array(select_boxs), -1, pred_dict['name'])
    ...
    def depth_callback(self, depth_msg:Image):
        self.proc_1.latest_depth = cv_bridge.imgmsg_to_cv2(depth_msg, desired_encoding='32FC1')

    

class Processor_ROS:
    def __init__(self, config_path, model_path, clock):
        self.points = None
    ...
    def set_pub_rviz(self, box3d_pub, marker_frame_id = 'map'):
        self.pub_rviz = Draw3DBox(box3d_pub, marker_frame_id, self.rate, self.get_clock)
